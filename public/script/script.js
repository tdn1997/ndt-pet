  ...
  $.ajax({
    ...
    success : function (result){
      if(result == 'LoginSuccess'){
        ...
        window.location.replace("http://localhost:8010/nguyen-duy-thuan/");
      } else {
  ...
  $.ajax({
   ...
   success : function (result){
    if(result == 'RegisterSuccess'){
      showAppModalBody('Tạo tài khoản thành công!');
      window.location.replace("http://localhost:8010/nguyen-duy-thuan/");
    } else{
      $('.errorMes')[0].style.display = "block";
      $('.errorMes').html(result);
...
function loadmore(start){
  ...
  $.ajax({
   ...
   success : function (result){
    if(!result){showAppModalBody("Đã hết sản phẩm để hiển thị!"); return 0;}
...
function editUserInfo(){
  ...
  $.ajax({
    ...
    success : function (result){
      showAppModalBody("Thay đổi thông tin thành công!");
      location.reload();
    }
  });
}
  ...
  $.ajax({
    ...
    success : function (result){
      if(result == 'Mật khẩu cũ sai!' || result == 'Nhập lại mật khẩu không trùng khớp!'){
        ...
        showAppModalBody("Đổi mật khẩu thành công!");
        location.reload();
      }
    }
  });
